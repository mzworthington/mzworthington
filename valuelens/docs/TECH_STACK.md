...
| Domain | **Zod** in `@valuelens/core` | ValueSpec as typed contract; mirror BlueprintSpec discipline |
...
| Local persistence | **File System Access API** + download/upload fallback | Folder = workspace; `valuetree.yaml` on disk |
...
```text
valuelens/
  app/                      # @valuelens/app — React SPA
  packages/core/            # @valuelens/core — ValueSpec, mismatches, pure logic
  packages/adapters/        # later: backstage, github, entra clients
  schemas/                  # JSON Schema published from Zod
  docs/
```

While nesting under `mzworthington/valuelens`, only **docs + schemas + samples + mockups** ship. Application packages appear when implementation starts (or when the dedicated repo is created).
...
|------------|------------|
| Backstage Catalog API | `adapters/backstage` — read Groups; write only ValueLens overlay kinds |
| GitHub teams + Actions metrics | Octokit; OAuth App / GitHub App |
| Entra groups | Microsoft Graph; Entra ID app registration |
| Sync / multi-device | Cloudflare Worker + R2 optional; still ValueSpec-first |
...
| Jira as SoR inside ValueLens | Dual entry kills adoption |
