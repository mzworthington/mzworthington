...
- Canonical **ValueSpec** as the only workspace source of truth
...
└───────────────┬───────────────────────────┬─────────────────┘
        ...
        └───────────────┘           └───────┬───────┘
             ...
             valuetree.yaml            (read Groups)     (read teams)
                                        overlay write*
* Slice 2+
```

## 3. Core domain (`@valuelens/core`)

Pure TypeScript + Zod. No React, no `fetch`, no filesystem.

| Module | Responsibility |
|--------|----------------|
| `valueSpec` | Schema, parse, serialize, migrate |
...
- `DetectMismatches`
- `ImportTeamDirectory` (Slice 2+) — merges refs into ValueSpec only
...
| Adapter | Port |
|---------|------|
| `FileSystemWorkspaceRepo` | Load/save `valuetree.yaml` via File System Access API |
...
| `BackstageOverlayWriter` | Emit `ValueBet` / overlay YAML only — **no Group YAML** if provider-backed |
...
IF team.provenance == provider THEN
  allow: ValueSpec, overlay entities, proposal report
...
| 2 | Tokens in browser memory / OS keychain patterns; never commit secrets to ValueSpec |
...
ValueLens never embeds ArchLens canvas. Deep link out is sufficient.

## 10. Quality gates

- `core` fully unit-tested (ValueSpec + mismatches)
