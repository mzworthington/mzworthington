...
ValueLens is an **interactive board pack**: edit strategy and topology intent in plain language, persist a canonical **ValueSpec**, export decision notes. It reads and later writes *around* existing systems of record; it does not become one.
...
| **ValueSpec** | Canonical YAML/JSON document for a workspace (`valuetree.yaml`) |
| **Workspace** | Local folder containing ValueSpec (+ optional exports) |
| **Overlay** (later) | Git-managed Backstage entities ValueLens owns — never replacement Groups for provider sync |
| **Board pack** | PDF/export for leadership distribution |

**Aggregate roots:** `Workspace` (contains ValueSpec document), `DecisionNote`.
...
5. **Git-friendly contract** — ValueSpec is diffable and reviewable even before CI exists.
...
Feature: Open a local workspace
  ...
  Scenario: Open existing folder
    Given a folder containing a valid valuetree.yaml
    When the leader opens that folder
    Then ValueLens loads the ValueSpec into steering and org views

  Scenario: Invalid ValueSpec
    Given a folder with an invalid valuetree.yaml
    When the leader opens that folder
...
Feature: Steering overview
  ...
  Scenario: Edit a bet
    ...
    Then the workspace ValueSpec is updated
...
- Unit: ValueSpec parse/validate/migrate; mismatch rules; merge of team display names
